...
Rpush.reflect do |on|
  ...
  on.apns_feedback do |feedback|
    device = AppleDeviceToken.find_by_token(feedback.device_token)
  ...
  on.gcm_canonical_id do |old_id, canonical_id|
    device = AndroidDeviceToken.find_by_token(old_id)
    device.token = canonical_id if canonical_id
  end
  ...
  on.gcm_invalid_registration_id do |app, error, registration_id|
    device = AndroidDeviceToken.find_by_token(registration_id)
    device.destroy if device
  end
